...
import type { Product, OrderItem, Quote } from "@/lib/admin/types";
...
export default function EditQuotePage() {
  ...
  const [form, setForm] = useState({ company: "", contact: "", email: "", phone: "", postcode: "", orderNotes: "" });
  ...
  useEffect(() => {
    fetch(`/api/admin/quotes/${id}`).then((r) => r.json()).then((q: Quote) => {
      ...
      const loadedItems: Partial<OrderItem>[] = q.items.length ? q.items : [blankItem()];
      setItems(loadedItems);
      setItemDrafts(loadedItems.map(draftFromItem));
      setDiscountText(q.discountPercent ? String(q.discountPercent) : "");
    });
  }, [id]);
  ...
  return (
    <div style={{ padding: "40px 40px 60px", maxWidth: 900 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 12 }}>
        <button onClick={() => router.push(`/admin/quotes/${id}`)} style={{ background: "none", border: "none", color: S.muted, cursor: "pointer", display: "flex", alignItems: "center", gap: 4, fontSize: "0.75rem" }}>
          <ArrowLeft size={14} /> Back to Quote
        </button>
      </div>
      <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 36, flexWrap: "wrap" }}>
        <h1 style={{ fontFamily: "var(--font-bebas, 'Bebas Neue', sans-serif)", fontSize: "1.8rem", letterSpacing: "0.06em", color: S.text, lineHeight: 1 }}>Edit Quote</h1>
        <span style={{ fontSize: "0.75rem", color: S.accent, fontFamily: "var(--font-jetbrains, monospace)" }}>{quote.reference}</span>
      </div>

      {quote.orderId && (
        <div style={{ background: "rgba(245,158,11,0.07)", border: "1px solid rgba(245,158,11,0.3)", padding: "13px 18px", marginBottom: 28, fontSize: "0.72rem", color: "#f59e0b", lineHeight: 1.6 }}>
          Heads up — this quote has already been converted to an order. Saving changes here updates the <em>quote</em> only;
          the linked order keeps its own copy of the items and pricing.{" "}
          <a href={`/admin/orders/${quote.orderId}`} style={{ color: "#f59e0b", textDecoration: "underline" }}>View the order →</a>
        </div>
      )}
      ...
      <div style={{ border: "1px solid #1a1a1a", padding: "16px", marginBottom: 24, background: "#0a0a0a", display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" }}>
        ...
        <p style={{ fontSize: "0.68rem", color: S.muted, maxWidth: 320, lineHeight: 1.5 }}>
          e.g. enter <strong style={{ color: S.text }}>20</strong> for 20% off — taken straight off the subtotal below.
        </p>
        {pct > 0 && (
      ...
      <div style={{ display: "flex", gap: 12 }}>
        ...
        <button
          onClick={() => router.push(`/admin/quotes/${id}`)}
          style={{ padding: "11px 22px", background: "transparent", border: "1px solid #2a2a2a", color: S.muted, fontSize: "0.72rem", cursor: "pointer" }}
        >
          Cancel
        </button>
      </div>
