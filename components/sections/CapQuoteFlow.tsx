...
import { getPositionPrice } from "@/lib/product-pricing";

/* ─── Constants ─── */
// Beanie: one size, one position (Front Centre), same price for either method
...
/* Cost price: £5.21 — blank sell price: £7.99 — tiers always stay above cost */
const QTY_TIERS: { min: number; max: number; price: number }[] = [
  { min: 10,  max: 24,       price: 7.99 },  // +£2.78  35% margin
  { min: 25,  max: 49,       price: 7.49 },  // +£2.28  30% margin
  { min: 50,  max: 99,       price: 6.99 },  // +£1.78  25% margin
  { min: 100, max: 249,      price: 6.49 },  // +£1.28  20% margin
  { min: 250, max: Infinity, price: 5.99 },  // +£0.78  13% margin
];

function unitPrice(q: number | ""): number {
  const n = Number(q) || 0;
  return QTY_TIERS.find(t => n >= t.min && n <= t.max)?.price ?? 7.99;
}
...
/* ─────────────────────────────────────────────────────────────────
   COMPONENT
───────────────────────────────────────────────────────────────── */
export default function CapQuoteFlow({ productName }: { productName: string }) {
  const basket = useBasket();

  const size = "One Size" as const;
  ...
  const handleAddToBasket = () => {
    ...
    basket.addItem({
      productName,
      productCode: "ELV-004",
      productHref: "/products/caps-beanies/workwear-cap",
      qty: numQty,
      totalQty: numQty,
      logo: logo!,
