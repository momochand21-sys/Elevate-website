...
/* ─── Constants ─── */
// Gilet — sizes available
const POSITIONS_LIST = ["Left Chest","Right Chest","Front Centre","Back"];
...
/* Cost price: £20.43 — blank sell price: £26.99 — tiers always stay above cost */
const QTY_TIERS: { min: number; max: number; price: number }[] = [
  { min: 10,  max: 24,       price: 26.99 },  // +£6.56  24% margin
  { min: 25,  max: 49,       price: 25.49 },  // +£5.06  20% margin
  { min: 50,  max: 99,       price: 23.99 },  // +£3.56  15% margin
  { min: 100, max: 249,      price: 22.49 },  // +£2.06   9% margin
  { min: 250, max: Infinity, price: 21.49 },  // +£1.06   5% margin
];

function unitPrice(q: number | ""): number {
  const n = Number(q) || 0;
  return QTY_TIERS.find(t => n >= t.min && n <= t.max)?.price ?? 26.99;
}
...
/* ─────────────────────────────────────────────────────────────────
   COMPONENT
───────────────────────────────────────────────────────────────── */
export default function GiletQuoteFlow({ productName }: { productName: string }) {
  ...
  const handleAddToBasket = () => {
    basket.addItem({
      productName,
      productCode: "ELV-005",
      productHref: "/products/gilets/workwear-gilet",
      sizeQtys: { ...sizeQtys },
      totalQty,
      logo: logo!,
