...
/* Cost price: £18.60 — tiers always stay above cost */
const QTY_TIERS: { min: number; max: number; price: number }[] = [
  { min: 10,  max: 24,       price: 25.99 },  // +£7.39  28% margin
  { min: 25,  max: 49,       price: 25.49 },  // +£6.89  27% margin
  { min: 50,  max: 99,       price: 23.99 },  // +£5.39  22% margin
  { min: 100, max: 249,      price: 22.49 },  // +£3.89  17% margin
  { min: 250, max: Infinity, price: 20.99 },  // +£2.39  11% margin
];

function unitPrice(q: number | ""): number {
  const n = Number(q) || 0;
  return QTY_TIERS.find(t => n >= t.min && n <= t.max)?.price ?? 25.99;
}
...
/* ─────────────────────────────────────────────────────────────────
   COMPONENT
───────────────────────────────────────────────────────────────── */
export default function HoodieQuoteFlow({ productName }: { productName: string }) {
  const basket = useBasket();

  /* Per-size quantities — e.g. { XS:0, S:5, M:10, L:5, XL:0, XXL:0 } */
  ...
  const handleAddToBasket = () => {
    basket.addItem({
      productName,
      productCode: "ELV-001",
      productHref: "/products/hoodies/heavyweight-hoodie",
      sizeQtys: { ...sizeQtys },
      totalQty,
      logo: logo!,
