...
/* Cost price: £4.83 — blank sell price: £6.99 — tiers always stay above cost */
const QTY_TIERS: { min: number; max: number; price: number }[] = [
  { min: 10,  max: 24,       price: 6.99 },  // +£2.16  31% margin
  { min: 25,  max: 49,       price: 6.49 },  // +£1.66  26% margin
  { min: 50,  max: 99,       price: 5.99 },  // +£1.16  19% margin
  { min: 100, max: 249,      price: 5.49 },  // +£0.66  12% margin
  { min: 250, max: Infinity, price: 5.19 },  // +£0.36   7% margin
];

function unitPrice(q: number | ""): number {
  const n = Number(q) || 0;
  return QTY_TIERS.find(t => n >= t.min && n <= t.max)?.price ?? 6.99;
}
...
/* ─────────────────────────────────────────────────────────────────
   COMPONENT
───────────────────────────────────────────────────────────────── */
export default function BeanieQuoteFlow({ productName }: { productName: string }) {
  ...
  const handleAddToBasket = () => {
    ...
    basket.addItem({
      productName,
      productCode: "ELV-003",
      productHref: "/products/caps-beanies/workwear-beanie",
      qty: numQty,
      totalQty: numQty,
      logo: logo!,
