...
/* Cost price: £18.00 — retail blank £24.99 — tiers always stay above cost */
const QTY_TIERS: { min: number; max: number; price: number }[] = [
  { min: 10,  max: 24,       price: 24.99 },  // +£6.99  28% margin
  { min: 25,  max: 49,       price: 23.99 },  // +£5.99  25% margin
  { min: 50,  max: 99,       price: 22.99 },  // +£4.99  22% margin
  { min: 100, max: 249,      price: 21.49 },  // +£3.49  16% margin
  { min: 250, max: Infinity, price: 19.99 },  // +£1.99  10% margin
];

function unitPrice(q: number | ""): number {
  const n = Number(q) || 0;
  return QTY_TIERS.find(t => n >= t.min && n <= t.max)?.price ?? 24.99;
}
...
/* ─────────────────────────────────────────────────────────────────
   COMPONENT
───────────────────────────────────────────────────────────────── */
export default function QuarterZipQuoteFlow({ productName }: { productName: string }) {
  const basket = useBasket();

  /* Per-size quantities — e.g. { S:5, M:10, L:5, XL:0, XXL:0 } */
  ...
  const reset = () => {
    setSizeQtys({XS:0,S:0,M:0,L:0,XL:0,XXL:0});
    setLogoResult(null); setBrandingResult(null);
  ...
  const handleAddToBasket = () => {
    basket.addItem({
      productName,
      productCode: "ELV-006",
      productHref: "/products/quarter-zip/workwear-quarter-zip",
      sizeQtys: { ...sizeQtys },
      totalQty,
      logo: logo!,
    ...
    return (
      ...
      >
        ...
        <p style={{ fontFamily:"var(--font-dm-sans,sans-serif)", fontSize:"0.85rem", color:"rgba(255,255,255,0.42)", lineHeight:1.7 }}>
          {productName} has been added to your basket. Configure more sizes or head to checkout.
        </p>
        <div className="flex flex-col gap-2">
          <Link href="/basket"
            style={{ fontFamily:"var(--font-jetbrains,monospace)", fontSize:"0.55rem", letterSpacing:"0.16em", textTransform:"uppercase", color:"#0041F9", textDecoration:"none" }}>
            View Basket →
          </Link>
              ...
              <div className="flex flex-wrap gap-1.5 mb-5">
                {[
                  ...
                  <div key={item.k} className="flex items-center gap-1.5 px-3 py-1.5"
                    ...
                    <span style={{ fontFamily:"var(--font-jetbrains,monospace)", fontSize:"0.44rem", letterSpacing:"0.14em", textTransform:"uppercase", color:"rgba(255,255,255,0.22)" }}>
                      {item.k}
                    </span>
                    <span style={{ fontFamily:"var(--font-jetbrains,monospace)", fontSize:"0.5rem", letterSpacing:"0.1em", textTransform:"uppercase", color:"rgba(255,255,255,0.65)" }}>
                      {item.v}
                    </span>
                  </div>
                ))}
